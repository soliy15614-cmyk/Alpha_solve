...
from ocr import process_ocr_captcha        # New OCR engine import
...
# ==================== CONFIGURATIONS ====================
BOT_TOKEN = "8803491256:AAF9C5e9nvZqHns5BAAU4vSDhi0F30KAI0E"
...
OCR_TOKENS = 0.8     # Fixed at 0.8 tokens
...
OCR_USD_COST = OCR_TOKENS / TOKEN_CONVERSION
...
async def remove_history_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Clears request_history for all users in the database. Restricted to ADMIN ONLY."""
    sender_id = update.effective_user.id
    chat_id = update.effective_chat.id
    user_msg_id = update.message.message_id
    ...
    status_msg = await update.message.reply_text("⏳ *Accessing database to wipe history vectors...*", parse_mode="Markdown")
    
    async with db_lock:
        try:
            db = await fetch_live_db(context)
            if not db:
                await status_msg.edit_text("⚠️ *Database is currently empty.*", parse_mode="Markdown")
                return

            # Loop through all users and reset only the request_history array
            for user_id in db:
                db[user_id]["request_history"] = []

            # Push updated state to cloud
            await commit_and_push_db(db, context)
            await status_msg.edit_text("✅ *Global Request History Purged!* All user history tracks have been cleared safely.", parse_mode="Markdown")
            print(f"{C_GREEN}[ADMIN ACTION] @{update.effective_user.username or sender_id} cleared all request history records.{C_RESET}")
            
        except Exception as e:
            await status_msg.edit_text(f"❌ *History Purge Failed:* `{str(e)}`", parse_mode="Markdown")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "• `/ocr <BASE64_PAYLOAD> or upload image with /ocr`\n"
        "• `/bypass <URL>`"
    )
...
async def price_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"🖼️ *OCR Captcha:* `{OCR_TOKENS} Token`\n\n"
...
async def ocr_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles OCR Captcha requests using text Base64, Direct Image upload, or Document Images with /ocr command/caption."""
    chat_id = update.effective_chat.id
    user_msg_id = update.message.message_id
    user_id = str(update.effective_user.id)
    username = update.effective_user.username
    ...
    # 1. Check if user uploaded a Compressed Photo/Image
    if update.message.photo:
        status_msg = await update.message.reply_text("📸 *Downloading and converting image to Base64...*", parse_mode="Markdown")
        try:
            photo_file = await update.message.photo[-1].get_file()
            image_buffer = BytesIO()
            await photo_file.download_to_memory(out=image_buffer)
            image_buffer.seek(0)
            base64_payload = base64.b64encode(image_buffer.read()).decode('utf-8')
        except Exception:
            await status_msg.edit_text("❌ *Failed to process uploaded image!*", parse_mode="Markdown")
            asyncio.create_task(schedule_delete_messages(context, chat_id, [user_msg_id, status_msg.message_id], delay=60))
            return

    # 2. Check if user uploaded an Uncompressed Document (PNG, JPG, JPEG)
    elif update.message.document and update.message.document.mime_type in ["image/png", "image/jpeg", "image/jpg"]:
        status_msg = await update.message.reply_text("📂 *Downloading and converting image document to Base64...*", parse_mode="Markdown")
        try:
            doc_file = await update.message.document.get_file()
            image_buffer = BytesIO()
            await doc_file.download_to_memory(out=image_buffer)
            image_buffer.seek(0)
            base64_payload = base64.b64encode(image_buffer.read()).decode('utf-8')
        except Exception:
            await status_msg.edit_text("❌ *Failed to process uploaded document image!*", parse_mode="Markdown")
            asyncio.create_task(schedule_delete_messages(context, chat_id, [user_msg_id, status_msg.message_id], delay=60))
            return

    # 3. Standard text command flow: /ocr <base64>
    else:
        args = context.args
        if len(args) < 1:
            msg = await update.message.reply_text("⚠️ Usage:\n• `/ocr <BASE64_PAYLOAD>`\n• Upload/Send image as file with `/ocr` as caption", parse_mode="Markdown")
            asyncio.create_task(schedule_delete_messages(context, chat_id, [user_msg_id, msg.message_id], delay=60))
            return
        base64_payload = args[0]
        status_msg = await update.message.reply_text("🔄 *Processing OCR Captcha Request...*", parse_mode="Markdown")

    # Balance Validation Logic
    user_data = await cloud_get_or_create_user(user_id, username, context)
    if user_data.get("balance", 0.0) < OCR_USD_COST:
        await status_msg.edit_text("❌ *Insufficient Balance!* Please refill your wallet.", parse_mode="Markdown")
        asyncio.create_task(schedule_delete_messages(context, chat_id, [user_msg_id, status_msg.message_id], delay=60))
        return

    try:
        # Executes the background engine process from ocr.py natively
        solved_token = await process_ocr_captcha(base64_payload)
    except Exception:
        solved_token = None

    # Strict Error Cloaking: Mask raw API failures into a generic message
    if solved_token and "error" not in str(solved_token).lower() and "fail" not in str(solved_token).lower():
        async with db_lock:
            db = await fetch_live_db(context)
            db[user_id]["balance"] = max(0.0, db[user_id]["balance"] - OCR_USD_COST)
            db[user_id]["requests_sent"] += 1
            db[user_id]["request_history"].append(datetime.utcnow().isoformat())
            await commit_and_push_db(db, context)

        await status_msg.edit_text(f"✅ *OCR Captcha Solved!*\n\n🔑 `{solved_token}`\n📉 `{OCR_TOKENS} Token Used`", parse_mode="Markdown")
    else:
        await status_msg.edit_text("❌ *OCR Solver Operations Failed! Check payload.*", parse_mode="Markdown")
...
def main():
    ...
    # OCR Handler - Supports Command, Compressed Photo, and Document File Streams
    app.add_handler(CommandHandler("ocr", ocr_handler, block=False))
    app.add_handler(MessageHandler(filters.PHOTO & filters.CaptionRegex(r"^/ocr"), ocr_handler, block=False))
    app.add_handler(MessageHandler(filters.Document.IMAGE & filters.CaptionRegex(r"^/ocr"), ocr_handler, block=False))
    ...
    app.add_handler(CommandHandler("remove", remove_history_handler, block=False)) # Added new /remove command
