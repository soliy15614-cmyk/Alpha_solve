...
async def set_balance_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Adds USD value converted to tokens to the user's existing balance."""
    sender_id = update.effective_user.id
...
async def turnstile_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Guarantees that both user command and final response (Success or Fail) delete in 60 seconds
